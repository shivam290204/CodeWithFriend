...
export default function Login() {
  ...
  return (
    <div className="flex min-h-screen bg-[#080420] text-[#E2E8F0] font-sans selection:bg-[#6366F1] selection:text-white">
      {/* Left Pane */}
      <div className="hidden lg:flex flex-1 relative bg-[#0c0630] border-r border-[#1E293B] items-center justify-center overflow-hidden grid-bg">
        {/* Logo Top Left */}
        <div className="absolute top-8 left-10 flex items-center gap-2">
          <img src="/logo.png" alt="CodeFlow" className="h-10 w-auto scale-125 origin-left object-contain" />
        ...
        <div className="bg-[#151822] border border-[#1E293B] rounded-xl shadow-2xl w-[450px] overflow-hidden opacity-90 hover:opacity-100 transition-opacity">
          <div className="h-10 border-b border-[#1E293B] flex items-center px-4 bg-[#080420]">
            <div className="flex gap-2">
              <div className="w-3 h-3 rounded-full bg-[#F87171]"></div>
              <div className="w-3 h-3 rounded-full bg-[#34D399]"></div>
              <div className="w-3 h-3 rounded-full bg-[#60A5FA]"></div>
            </div>
            <span className="ml-4 text-xs font-mono text-[#94A3B8]">auth_service.rs</span>
          </div>
          <div className="p-6 font-mono text-sm leading-relaxed text-[#94A3B8]">
            <span className="text-[#34D399]">pub async fn</span> <span className="text-[#E2E8F0]">authenticate_user</span>(<br/>
            &nbsp;&nbsp;req: AuthRequest<br/>
            ) -&gt; <span className="text-[#60A5FA]">Result</span>&lt;AuthResponse, AuthError&gt; {"{"}<br/>
            &nbsp;&nbsp;<span className="text-[#475569]">// Validate credentials against db</span><br/>
            &nbsp;&nbsp;<span className="text-[#C084FC]">let</span> user = db::users::find(&amp;req.email).<span className="text-[#60A5FA]">await</span>?;<br/><br/>
            &nbsp;&nbsp;<span className="text-[#C084FC]">if</span> !crypto::verify(&amp;req.password, &amp;user.hash) {"{"}<br/>
            &nbsp;&nbsp;&nbsp;&nbsp;<span className="text-[#C084FC]">return</span> <span className="text-[#F87171]">Err</span>(AuthError::InvalidCredentials);<br/>
            &nbsp;&nbsp;{"}"}<br/><br/>
            &nbsp;&nbsp;<span className="text-[#C084FC]">let</span> token = jwt::generate(&amp;user.id)?;<br/>
            &nbsp;&nbsp;<span className="text-[#34D399]">Ok</span>(AuthResponse {"{ token }"})<br/>
            {"}"}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 sm:p-12 relative">
        <Link to="/" className="lg:hidden absolute top-8 left-8 flex items-center gap-2">
          <img src="/logo.png" alt="CodeFlow" className="h-8 w-auto scale-125 origin-left object-contain" />
        ...
        <div className="w-full max-w-md">
          <div className="text-center mb-10">
            ...
            <p className="text-[#94A3B8] text-sm">Sign in to continue your workflow.</p>
          </div>
          ...
          <div className="flex bg-[#151822] border border-[#1E293B] p-1 rounded-lg mb-8">
            ...
            <Link to="/signup" className="flex-1 py-2 text-sm font-semibold rounded-md text-[#94A3B8] hover:text-white text-center transition-colors">
              Signup
            </Link>
          </div>
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex justify-between items-center">
                ...
                <span className="text-xs text-[#6366F1] hover:text-[#818CF8] cursor-pointer">Forgot?</span>
              </div>
              <div className="relative">
            ...
            <button className="mt-2 w-full bg-[#BFDBFE] hover:bg-[#93C5FD] text-[#1E3A8A] font-bold py-3 rounded-md flex items-center justify-center gap-2 transition-colors">
              Enter the Flow <ArrowRight className="w-4 h-4" />
            </button>
          </form>
          ...
          <div className="flex gap-4">
            <button className="flex-1 bg-[#080420] border border-[#1E293B] hover:bg-[#1E293B] text-white py-2.5 rounded-md flex items-center justify-center gap-2 text-sm transition-colors">
