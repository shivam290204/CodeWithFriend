...
const sourceHome = path.join(__dirname, '..', 'src', 'app', 'page.tsx');
if (fs.existsSync(sourceHome)) {
  const homeContent = replaceNextJs(sourceHome, false);
...
const sourceRoom = path.join(__dirname, '..', 'src', 'app', 'room', '[roomCode]', 'page.tsx');
if (fs.existsSync(sourceRoom)) {
  const roomContent = replaceNextJs(sourceRoom, true);
  fs.mkdirSync(path.join(__dirname, 'src', 'pages'), { recursive: true });
